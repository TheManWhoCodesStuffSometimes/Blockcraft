import { Building, BuildingType, EntityBase, EntityId, GameState, GRID_H, GRID_W, Logs, Owner, Systems, Tile, TICK_MS, Unit, UnitType } from "./types";
...
function spawnBuilding(gs: GameState, btype: BuildingType, owner: Owner, x: number, y: number): Building {
  const id = gs.nextId++;
  const size = btype === BuildingType.CommandCenter ? 2 : 2;
  const maxHp = btype === BuildingType.CommandCenter ? 600 : 400;
  const b: Building = {
    id, owner, x, y, hp: maxHp, maxHp,
    kind: "building", btype, size, queue: [], queueProgress: 0
  };
...
function spawnUnit(gs: GameState, utype: UnitType, owner: Owner, x: number, y: number): Unit {
  const id = gs.nextId++;
  let hp = 0, atk = 0, range = 1, speed = 4;
  if (utype === UnitType.Worker) { hp = 60; atk = 2; range = 1; speed = 4; }
  if (utype === UnitType.Melee) { hp = 80; atk = 8; range = 1; speed = 5; }
  if (utype === UnitType.Ranged) { hp = 60; atk = 6; range = 3; speed = 4; }
  const u: Unit = {
    id, owner, x, y, hp, maxHp: hp,
    kind: "unit", utype, speed, attack: atk, range,
    attackCooldown: 0,
...
export function createInitialState(): GameState {
  ...
  const players: Record<Owner, any> = { player: { gold: 50 }, ai: { gold: 50 } };
  const logs: Logs = {
    events: [],
    summary() {
      return {
        version: "0.1",
        events: this.events.slice(-500), // trimmed
  ...
  const systems: Systems = {
    step(gs: GameState) {
      stepEconomy(gs);
      stepProduction(gs, spawnUnit);
      stepCombatAndMovement(gs);
      gs.tick++;
    },
  };
  const gs: GameState = {
    tick: 0, rng, tiles, entities, units, buildings, nextId: 1,
    players,
...
export function selectEntityAt(gs: GameState, tx: number, ty: number, owner: Owner) {
  ...
  for (const id of gs.buildings) {
    ...
    const within = tx >= b.x && tx < b.x + b.size && ty >= b.y && ty < b.y + b.size;
    if (within) { best = { id: b.id, d: 0 }; break; }
  }
...
export function rightClickCommand(gs: GameState, tx: number, ty: number) {
  ...
  const u = e as Unit;
  // workers: if right click on resource, set harvest target
...
export function tryPlaceBarracks(gs: GameState, tx: number, ty: number, owner: Owner) {
  ...
  // check 2×2 free and not out of bounds
  if (tx + 1 >= GRID_W || ty + 1 >= GRID_H) return { ok: false, msg: "Out of bounds" };
  ...
  p.gold -= 100;
  gs.logs.events.push({ t: gs.tick, type: "place_barracks", payload: { owner, tx, ty } });
  return { ok: true, msg: "Barracks placed" };
}

export function trainUnit(gs: GameState, buildingType: BuildingType, utype: UnitType, owner: Owner) {
  // costs
  const cost = utype === UnitType.Worker ? 50 : utype === UnitType.Melee ? 30 : 40;
  const time = utype === UnitType.Worker ? 60 : utype === UnitType.Melee ? 40 : 50; // ticks
  const p = gs.players[owner];
  if (p.gold < cost) return { ok: false, msg: "Not enough gold" };

  // find a building of that type
  const b = [...gs.buildings].map(id => gs.entities.get(id) as Building)
    .find(b => b.owner === owner && b.btype === buildingType);
  if (!b) return { ok: false, msg: `Need ${buildingType}` };

  b.queue.push(utype);
  // store time in queue by repeating entries? keep simple: use queueProgress per system
  // pay now
  p.gold -= cost;
  gs.logs.events.push({ t: gs.tick, type: "queue", payload: { owner, building: buildingType, utype } });
  return { ok: true, msg: `${utype} queued (${time} ticks)` };
}

export function spawnFromBuilding(gs: GameState, b: Building, utype: UnitType) {
  // spawn adjacent if possible
  const spots = [
    { x: b.x + b.size, y: b.y }, { x: b.x - 1, y: b.y },
    { x: b.x, y: b.y + b.size }, { x: b.x, y: b.y - 1 },
  ];
  for (const s of spots) {
    ...
    gs.logs.events.push({ t: gs.tick, type: "spawn", payload: { owner: b.owner, utype, x: s.x, y: s.y } });
    return u;
  }
  return null;
}
